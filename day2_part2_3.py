...
#use a collection to create minimum info and then derive everything else 
Item = collections.namedtuple('Item', ['name', 'score','elf_map','loses_to','beats'])
items=[]
items.append(Item('Rock',1,'A','Paper','Scissors'))
items.append(Item('Paper',2,'B','Scissors','Rock'))
items.append(Item('Scissors',3,'C','Rock','Paper'))
...
for i in range(len(items)):
    me=items[i].name
    item_scores[me]=items[i].score
    elf_map[items[i].elf_map]=me
    item_rank[me+"/"+me]=0
    item_rank[me+"/"+items[i].loses_to]=1
    item_rank[me+"/"+items[i].beats]=-1
